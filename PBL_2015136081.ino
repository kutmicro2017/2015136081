...
delay(60000); //1분간 대기
digitalWrite(LED3_G,LOW); digitalWrite(LED4_G, LOW); //초록색 신호를 끈다
for(int delaytime=0; delaytime <10; delaytime++){ //10초간
digitalWrite(LED3_Y,HIGH); digitalWrite(LED4_Y, HIGH); //노란 신호가 들어옴
delay(500);  
digitalWrite(LED3_Y,LOW); digitalWrite(LED4_Y, LOW); //노란 신호가 꺼짐
...
unsigned long previousMillis = millis(); //현재 시각을 저장
//millis()는 아두이노에 전원이 들어온 후부터 지난 시각을 밀리초 단위로 반환
...
} //상하 신호등과 연동되는 보행자 신호이므로 상하 신호등이 초록색일 때만 동작하면 된다
